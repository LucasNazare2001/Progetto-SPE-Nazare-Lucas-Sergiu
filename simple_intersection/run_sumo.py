...
if __name__ == "__main__":
    ...
    if "fixed" in strategy_results and "adaptive" in strategy_results:
        ...
        print(f"\n{'IMPROVEMENTS FIXED VS ADAPTIVE:':<50}")
        print("-" * 50)
        print(f"Waiting time reduction:   {(a['waiting'] - f['waiting']) / f['waiting'] * 100:+.1f}%")
        print(f"Duration reduction:       {(a['duration'] - f['duration']) / f['duration'] * 100:+.1f}%")
        print(f"Time loss reduction:      {(a['loss'] - f['loss']) / f['loss'] * 100:+.1f}%")
